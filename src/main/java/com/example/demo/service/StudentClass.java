...
public class StudentClass {
	...
	private StudentList<String> studentList;
	private ScoreList<Double> scoreList;
	public StudentClass() {
	...
	public String getQuery(String studentData,String query){
		...
		int type=Integer.parseInt(queries[0]);
		String result="";
		if(type==1) {
			for(String stu:students) {
				studentList.addElement(stu);
			}
			result=studentList.beginsWith(queries[1]);

		}else if(type==2) {
			for(String stu:students) {
				studentList.addElement(stu);
			}
			String bloodGroups[]=queries[1].split(" ");
			String find=queries[2];
			result=studentList.bloodGroupOf(bloodGroups, find);
		}else if(type==3) {
			StudentList<Integer> studentScore=new StudentList<>();
			for(String stu:students) {
				studentScore.addElement(Integer.parseInt(stu));
			}
			int threshold=Integer.parseInt(queries[1]);
			int count=studentScore.thresholdScore(threshold);
			result=count+" students scored "+threshold+" above";
		}else if(type==4) {
