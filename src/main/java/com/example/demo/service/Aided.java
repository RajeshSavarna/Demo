...
public class Aided extends Student{
	...
	public String result(String allMarks) {
		...
		String subjectMarks[]=marks[0].split(",");
		String nccMarks=marks[1];
		String sportsMarks=marks[2];
		Double gradeCreditPoint=0.0;
		Integer totalCreditPoint=0;
		for(int i=0;i<subjectMarks.length;i++){
			String str[]=subjectMarks[i].split(" ");
			Integer subMarks=Integer.parseInt(str[0]);
			Integer creditPoints=Integer.parseInt(str[1]);
			gradeCreditPoint+= (gradePoint(subMarks)*creditPoints);
			totalCreditPoint+=5;
		}
		String ncc[]=nccMarks.split(",");
		Integer first=Integer.parseInt(ncc[0]);
		Integer mark=Integer.parseInt(ncc[1]);
		Integer credits=Integer.parseInt(ncc[2]);
		if(first!=0){
			gradeCreditPoint+=(gradePoint(mark)*credits);
			totalCreditPoint+=5;
		}



		String sports[]=sportsMarks.split(",");
		Integer frst=Integer.parseInt(sports[0]);
		Integer mrk=Integer.parseInt(sports[1]);
		Integer credt=Integer.parseInt(sports[2]);
		if(frst!=0){
			gradeCreditPoint+=(gradePoint(mrk)*credt);
			totalCreditPoint+=5;
		}



		return String.format("%.2f", (gradeCreditPoint/totalCreditPoint) );
