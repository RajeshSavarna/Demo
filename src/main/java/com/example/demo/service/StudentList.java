...
public class StudentList<T> {
	...
	public String beginsWith(String s) {
		String result="";
		for (T str : studentList) {
			String val = String.valueOf(str);
			if (val.toLowerCase().startsWith(s.toLowerCase()))
				result+=val+"\n";
		}
		return result;
	}
	...
	public String bloodGroupOf(String[] bloodGroups, String find) {
		String result="";
		for (int i = 0; i < bloodGroups.length; i++) {
			if (bloodGroups[i].equals(find)) {
	...
	public int thresholdScore(int score) {
		int count = 0;
		for (T sc : studentList) {
			int marks = Integer.parseInt(String.valueOf(sc));
			if (marks >= score) {
				count++;
			}
		}
